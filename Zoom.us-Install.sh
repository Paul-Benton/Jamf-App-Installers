...
if [ '`/usr/bin/uname -p`'="i386" -o '`/usr/bin/uname -p`'="x86_64" ]; then
        ...
        if [ "${currentinstalledver}" != "${latestver}" ]; then
				...
				echo "Attempting to quit existing Zoom"
				killall "zoom.us"
                sleep 5
				echo "Deleting any existing Zoom"
				rm -R /Applications/zoom.us.app
				rm -R /Users/*/Applications/zoom.us.app
        ...
        if [ "${latestver}" = "${newlyinstalledver}" ]; then
               ...
               /Library/Application\ Support/JAMF/bin/jamfHelper.app/Contents/MacOS/jamfHelper -windowType hud -windowPosition  ur -icon  /Applications/zoom.us.app/Contents/Resources/ZPLogo.icns -heading "Zoom Installed" -description "Zoom.us has been updated." -timeout 30 &
